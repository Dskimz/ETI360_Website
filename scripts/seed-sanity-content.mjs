...
async function main() {
  ...
  if (!projectId) throw new Error('Missing env var: NEXT_PUBLIC_SANITY_PROJECT_ID')
  if (!token) {
    throw new Error(
      'Missing env var: SANITY_WRITE_TOKEN (or SANITY_READ_TOKEN with write permissions)',
    )
  }
  ...
  const docs = [
    ...
    pageDoc({
      ...
      sections: [
        ...
        toCta({headline: 'Start a conversation', bodyMarkdown: 'Discuss how TripRisk360 supports your program.'}),
      ],
    }),
    insightDoc({
  ...
  for (const doc of docs) {
    // Create/replace published docs directly
    // eslint-disable-next-line no-await-in-loop
    await client.createOrReplace(doc)
    created += 1
  }

  console.log(`Seeded ${created} documents into ${projectId}/${dataset}`)
}
